...
def plot_ray(dists, ys, fig=None, z_sag=None, color="red", linewidth=1, dashtype='-'):
    ...
    for i in range(1, len(dists)):
        if not np.isinf(Rs[i]):
            ...
            for pm in [+1,-1]:
                ...
                fig.axes[0].plot(vertex_pos+np.sign(Rs[i])*zz, yy, color="k", linewidth=2)
                if pm == +1:
                    lens_edges.append([vertex_pos+np.sign(Rs[i])*zz[-1], yy[-1]])
            if not nmat[i]:
